...
const AdminLayout = () => {
    ...
    const allMenuItems = [
        ...
        { path: "/staff", icon: LayoutDashboard, label: "Dashboard", roles: ['Staff'] },
        { path: "/admin/users", icon: Users, label: "Quản lý người dùng", roles: ['Admin'] },
        { path: "/staff/donors", icon: Users, label: "Quản lý Người hiến máu", roles: ['Staff'] },
        ...
        { path: "/staff/inventory", icon: Warehouse, label: "Kho máu", roles: ['Staff'] },
        { path: "/staff/requests", icon: MessageSquareWarning, label: "Yêu cầu", roles: ['Staff'] }
    ];

    // Lọc menu dựa trên vai trò của user
    const menuItems = allMenuItems.filter(item => item.roles.includes(user?.role));

    const panelTitle = user?.role === 'Admin' ? 'Admin Panel' : 'Staff Panel';
    const homeLink = user?.role === 'Admin' ? '/admin' : '/staff';
        ...
        <div className="flex h-screen bg-gray-100">
            ...
            <aside className="w-64 bg-gray-800 text-white flex flex-col fixed h-full">
                <div className="flex items-center justify-center h-16 border-b border-gray-700">
                    <Link to={homeLink} className="flex items-center space-x-2 text-white">
