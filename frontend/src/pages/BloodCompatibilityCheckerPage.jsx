...
const BloodCompatibilityCheckerPage = () => {
    ...
    useEffect(() => {
        const fetchBloodData = async () => {
            try {
                setBloodData(prev => ({ ...prev, isLoading: true, error: null }));

                const [typesResponse, rulesResponse] = await Promise.all([
                    bloodService.getBloodTypes(),
                    bloodService.getAllBloodCompatibility()
                ]);
                ...
                setBloodData(prev => ({
                    ...prev,
                    isLoading: false,
                    error: 'Failed to load blood compatibility data'
                }));
            }
        };
    ...
    // Thay vì nhiều state, dùng computed values
    const wholeBloodTypes = useMemo(
        () => bloodData.allTypes.filter(type => type.category === 'Whole Blood'),
        [bloodData.allTypes]
    );

    const bloodGroups = useMemo(
        () => [...new Set(bloodData.allTypes.map(type => type.group))],
        [bloodData.allTypes]
    );

    const componentTypes = useMemo(
        () => [...new Set(bloodData.allTypes.map(type => type.componentType))].filter(c => c !== 'Whole Blood'),
        [bloodData.allTypes]
    );

    useEffect(() => {
        if (wholeBloodTypes.length > 0) {
            setSelectedWholeBloodId(String(wholeBloodTypes[0].id));
        }
        if (bloodGroups.length > 0) {
            setSelectedBloodGroup(bloodGroups[0]);
        }
        if (componentTypes.length > 0) {
            setSelectedComponent(componentTypes[0]);
        }
    }, [wholeBloodTypes, bloodGroups, componentTypes]);
    ...
    useEffect(() => {
        ...
        if (activeSubTab === 'whole') {
            ...
            const foundType = bloodData.allTypes.find(bt => bt.bloodGroup === selectedBloodGroup && bt.componentType === selectedComponent);
            if (foundType) {
                targetTypeId = foundType.id;
            }
        }

        if (targetTypeId && bloodData.rules.length > 0) {
            const donors = new Set();
            bloodData.rules.forEach(rule => {
    ...
    }, [selectedWholeBloodId, selectedBloodGroup, selectedComponent, activeSubTab, bloodData.allTypes, bloodData.rules]);
    ...
    const renderCompatibilityTab = () => (
        <div className="animate-modal-appear space-y-8">
            ...
            {bloodData.isLoading ? <div className="flex justify-center items-center py-10"><LoadingSpinner /></div> : (
