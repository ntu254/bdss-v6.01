...
const AdminBloodInventoryPage = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            const [inventoryResponse, bloodTypesResponse] = await Promise.all([
                inventoryService.getAll(), // Thay bằng service tương ứng
                bloodService.getBloodTypes()
            ]);
            setInventory(inventoryResponse.data.content); // Assuming pagination
            setBloodTypes(bloodTypesResponse); // Service đã trả về .data
        } catch (error) {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        const toastId = toast.loading(editingItem ? 'Đang cập nhật...' : 'Đang thêm...');
        try {
            if (editingItem) {
                await inventoryService.updateInventory(editingItem.id, formData);
                toast.success('Cập nhật kho máu thành công', { id: toastId });
            } else {
                await inventoryService.createInventory(formData);
                toast.success('Thêm kho máu thành công', { id: toastId });
            }
            ...
            toast.error(error.response?.data?.message || 'Lỗi khi lưu dữ liệu kho máu', { id: toastId });
        }
    };
